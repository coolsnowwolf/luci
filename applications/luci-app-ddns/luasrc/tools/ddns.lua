...
if nixiofs.stat('/tmp/lua_curlproxy_check', 'type') == 'reg' then
	has_curlpxy	= nixio.fs.readfile('/tmp/lua_curlproxy_check')
else
	has_curlpxy	= (SYS.call( [[grep -i "all_proxy" /usr/lib/libcurl.so* >/dev/null 2>&1]] ) == 0)
	nixio.fs.writefile('/tmp/lua_curlproxy_check', has_curlpxy)
endif
