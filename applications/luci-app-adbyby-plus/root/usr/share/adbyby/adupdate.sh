...
touch /tmp/md5.json && uclient-fetch --no-check-certificate -T 10 -O /tmp/md5.json http://adbyby.coding.net:8080/p/xwhyc-rules/d/xwhyc-rules/git/raw/master/md5.json
...
if [ "$lazy_online"x != "$lazy_local"x -o "$video_online"x != "$video_local"x ]; then
    ...
    touch /tmp/lazy.txt && uclient-fetch --no-check-certificate -T 10 -O /tmp/lazy.txt http://adbyby.coding.net:8080/p/xwhyc-rules/d/xwhyc-rules/git/raw/master/lazy.txt
    touch /tmp/video.txt && uclient-fetch --no-check-certificate -T 10 -O /tmp/video.txt http://adbyby.coding.net:8080/p/xwhyc-rules/d/xwhyc-rules/git/raw/master/video.txt
