...
if [ ! -f "/tmp/adbyby.updated" ];then
  ...
  while [ "$wget_ok" = "0" ] 
    ...
    if [ "$?" == "0" ]; then
      ...
      touch /tmp/md5.json && uclient-fetch --no-check-certificate -t 1 -T 10 -O /tmp/md5.json http://adbyby.coding.net:8080/p/xwhyc-rules/d/xwhyc-rules/git/raw/master/md5.json
      ...
      if [ "$adm5" == "$bmd5" ];then
        ...
        touch /tmp/lazy.txt && uclient-fetch --no-check-certificate -t 1 -T 10 -O /tmp/lazy.txt http://adbyby.coding.net:8080/p/xwhyc-rules/d/xwhyc-rules/git/raw/master/lazy.txt
        touch /tmp/video.txt && uclient-fetch --no-check-certificate -t 1 -T 10 -O /tmp/video.txt http://adbyby.coding.net:8080/p/xwhyc-rules/d/xwhyc-rules/git/raw/master/video.txt
